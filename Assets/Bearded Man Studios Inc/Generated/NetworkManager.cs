...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] CubeSpinnerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is CubeSpinnerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (CubeSpinnerNetworkObject.Length > 0 && CubeSpinnerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(CubeSpinnerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<CubeSpinnerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateCubeSpinner instead, its shorter and easier to type out ;)")]
		public CubeSpinnerBehavior InstantiateCubeSpinnerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(CubeSpinnerNetworkObject[index]);
			var netBehavior = go.GetComponent<CubeSpinnerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<CubeSpinnerBehavior>().networkObject = (CubeSpinnerNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of CubeSpinner
		/// </summary>
		/// <returns>
		/// A local instance of CubeSpinnerBehavior
		/// </returns>
		/// <param name="index">The index of the CubeSpinner prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public CubeSpinnerBehavior InstantiateCubeSpinner(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(CubeSpinnerNetworkObject[index]);
			var netBehavior = go.GetComponent<CubeSpinnerBehavior>();
